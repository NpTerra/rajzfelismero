...
//----------------------------------------------------------------------------------
GuiWindowFileDialogState InitGuiWindowFileDialog(const char *initPath, int width, int height);
...
GuiWindowFileDialogState InitGuiWindowFileDialog(const char *initPath, int width, int height)
{
    ...
    // Init window data
    state.windowBounds = (Rectangle){ GetScreenWidth()/2 - width/2, GetScreenHeight()/2 - height/2, width, height };
...
void FreeDialog(GuiWindowFileDialogState *state)
{
    if(state->isFreed) return;
...
# endif
        ...
        if (state->SelectFilePressed || state->CancelFilePressed) state->windowActive = false;
